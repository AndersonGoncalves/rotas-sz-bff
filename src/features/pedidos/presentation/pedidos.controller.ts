...
export class PedidosController extends BaseRouter {
  ...
  applyRoutes(application: restify.Server): void {
    ...
    application.get('/pedidos/:id', async (req, res, next) => {
    ...
    application.put('/pedidos/:id', async (req, res, next) => {
      try {
    ...
    application.patch('/pedidos/:id', async (req, res, next) => {
      try {
    ...
    application.del('/pedidos/:id', async (req, res, next) => {
      try {
        if (!(await this.repo.delete(req.params.id))) return next(new NotFoundError('Pedido não encontrado'));
        res.send(204);
        return next();
      } catch (e) { return next(e); }
    });
