...
export class ChecklistAssistenciaController extends BaseRouter {
  ...
  applyRoutes(application: restify.Server): void {
    application.get('/checklist-assistencia-agua-gelada', async (req, res, next) => {
      try { res.json(await this.repo.findAll()); return next(); }
      catch (e) { return next(e); }
    });

    application.get('/checklist-assistencia-agua-gelada/:id', async (req, res, next) => {
      try { this.render(res, next)(await this.repo.findById(req.params.id)); }
      catch (e) { return next(e); }
    });

    application.post('/checklist-assistencia-agua-gelada', async (req, res, next) => {
      try {
    ...
    application.patch('/checklist-assistencia-agua-gelada/:id', async (req, res, next) => {
      try {
