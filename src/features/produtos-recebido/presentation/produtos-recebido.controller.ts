...
export class ProdutosRecebidoController extends BaseRouter {
  ...
  applyRoutes(application: restify.Server): void {
    application.get('/produtos-recebido', async (req, res, next) => {
      try { res.json(await this.repo.findAll()); return next(); }
      catch (e) { return next(e); }
    });

    application.get('/produtos-recebido/:id', async (req, res, next) => {
      try { this.render(res, next)(await this.repo.findById(req.params.id)); }
      catch (e) { return next(e); }
    });

    application.post('/produtos-recebido', async (req, res, next) => {
      try {
    ...
    application.patch('/produtos-recebido/:id', async (req, res, next) => {
      try {
